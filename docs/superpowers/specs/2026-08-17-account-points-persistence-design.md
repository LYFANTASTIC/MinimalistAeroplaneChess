...
本次改造取消完整的局内道具体系。撞机产生的积分不再进入单局资源池，也不再用于购买道具，而是直接累计到已登录玩家的永久账户积分。

## 已确认的产品规则

- 部署面向中国大陆玩家：Node/WebSocket 服务和 Supabase 项目均放在新加坡区域。
- 保留现有自建用户名、邮箱和密码登录，不迁移到 Supabase Auth。
- 删除道具模式开关、四种道具、局内积分条、100 分上限和积分消费。
- 普通模式撞掉敌方飞机时获得账户积分。
...
- 移除 `energyChange`、`energyGainAnimation`、`teleportIcon`、`polyhedralDice`、`mysteryBoxIcon` 等道具/局内积分消息。
...
- 删除大厅三个配置区中的道具模式开关。
- 删除道具规则说明。
- 删除游戏页道具按钮、道具面板、局内积分条和积分调试控件。
- 删除盲盒、传送门、多面骰子、遥控骰子及相关动画和 AI 决策。
- 撞机或欢乐碰撞后显示“账户积分 +X”，到账后更新账户余额。
- 结算页保留积分获得统计，名称改为“本局账户积分”。
- 删除锦鲤附体、慈善家、天命之子、倒霉熊、道具大师和次元旅人等道具称号。
